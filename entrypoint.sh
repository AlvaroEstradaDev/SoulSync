...
# Application directories — single source of truth for all mkdir/chown/writability checks
APP_DIRS=(
    /app/config
    /app/data
    /app/logs
...
if [ "$CURRENT_UID" != "$PUID" ] || [ "$CURRENT_GID" != "$PGID" ]; then
    ...
    if [ "$DATA_OWNER" != "$PUID:$PGID" ]; then
        ...
        chown -R soulsync:soulsync "${APP_DIRS[@]}" 2>/dev/null || true
    else
...
mkdir -p "${APP_DIRS[@]}" 2>/dev/null || true
chown soulsync:soulsync "${APP_DIRS[@]}" 2>/dev/null || true
